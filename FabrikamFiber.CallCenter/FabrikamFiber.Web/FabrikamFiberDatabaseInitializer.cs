namespace FabrikamFiber.Web
{
    ...
    {
        private int extraServiceTicketsToCreate = 1000;
        protected override void Seed(FabrikamFiberWebContext context)
        {
            ...
            //adding in many closed tickets to create a performance load.

            for (int i = 0; i < extraServiceTicketsToCreate; i++)
            {
                serviceTicket = context.ServiceTickets.Add(new ServiceTicket
                {
                    Title = "Channels gone!",
                    ...
                    CreatedBy = employees[1],
                    AssignedTo = employees[2],
                    Opened = DateTime.Now.AddDays(-3),
                    Closed = DateTime.Now.AddDays(-1),
                });
            }

            //finished adding extra closed tickets
